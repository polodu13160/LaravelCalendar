<x-action-section>
    ...
    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Si nécessaire, vous pouvez vous déconnecter de toutes vos autres sessions de navigateur sur tous vos
            appareils. Certaines de vos sessions récentes sont listées ci-dessous ; cependant, cette liste peut ne pas
            être exhaustive. Si vous pensez que votre compte a été compromis, vous devriez également mettre à jour votre
            mot de passe.') }}
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ms-3">
                    <div class="text-sm text-gray-600">
                        {{ $session->agent->platform() ? $session->agent->platform() : __('Inconnu') }} - {{
                        $session->agent->browser() ? $session->agent->browser() : __('Inconnu') }}
                    </div>
