    ...
    public function handle()
    {
        ...
        foreach ($events as $event) {
            try {
                ...
                if ($ics == null) {
                    $event->createIcs();
                } else if ($ics->lastmodified > $event->updated_at->getTimestamp()) {
                    $event->modificationIcsToEvent();
                } else if ($ics->lastmodified < $event->updated_at->getTimestamp()) {
                    $event->modificationEventToIcs();
                }

                Log::channel('events-ics')->info('Succès : Mise à jour réussie pour l\'événement ID ' . $event->id);


            } catch (\Exception $e) {

                Log::channel('events-ics')->error('Échec : Impossible de mettre à jour l\'événement ID ' . $event->id . '. Erreur : ' . $e->getMessage());
                $x++;
